...
void cb(const action_demo::AddIntsGoalConstPtr &goal,Server* server){
    ...
    //累加并响应连续反馈
    int result = 0;
    action_demo::AddIntsFeedback feedback;//连续反馈
...
int main(int argc, char *argv[])
{
    ...
    /*SimpleActionServer(ros::NodeHandle n, 
                        std::string name, 
                        boost::function<void (const action_demo::AddIntsGoalConstPtr &)> execute_callback, 
                        bool auto_start)
    */
    // actionlib::SimpleActionServer<action_demo::AddIntsAction> server(....);
